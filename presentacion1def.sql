...
--rule update
CREATE OR REPLACE RULE updateEnTareas AS
ON UPDATE TO sc_pruebas.tablaTareas
DO INSTEAD
UPDATE sc_pruebas.tareasSecundaria
SET 
    id_proyecto = NEW.id_proyecto,
    nombre_tarea = NEW.nombre_tarea,
    legajo_alumno = NEW.legajo_alumno,
...
--tercera rule delete
CREATE OR REPLACE RULE deleteEnTareas AS
ON DELETE TO sc_pruebas.tablaTareas
DO INSTEAD
DELETE FROM sc_pruebas.tareasSecundaria
WHERE id_tarea = OLD.id_tarea;
--ejemplo
...
--delete
DELETE FROM sc_pruebas.tablaTareas
WHERE id_tarea = 1;
